...
class Block {
    ...
    static async genWithdrawalsTrieRoot(wts, emptyTrie) {
        ...
        for (const [i, wt] of wts.entries()) {
            await trie.put(Buffer.from(rlp_1.RLP.encode(i)), (0, util_1.arrToBufArr)(rlp_1.RLP.encode(wt.raw())));
        }
        return trie.root();
    }
    /**
    ...
    static async generateWithdrawalsSSZRoot(withdrawals) {
        util_1.ssz.Withdrawals.hashTreeRoot(withdrawals.map((wt) => wt.toValue()));
    }
    /**
            ...
            if (this._common.isActivatedEIP(4844) === true) {
                if (tx instanceof tx_1.BlobEIP4844Transaction) {
                    ...
                    if (blockDataGas > dataGasLimit) {
                        errs.push(`tx causes total data gas of ${blockDataGas} to exceed maximum data gas per block of ${dataGasLimit}`);
                    }
                }
            }
            if (errs.length > 0) {
    ...
    validateBlobTransactions(parentHeader) {
        for (const tx of this.transactions) {
            if (tx instanceof tx_1.BlobEIP4844Transaction) {
                const dataGasPrice = (0, helpers_1.getDataGasPrice)(parentHeader);
                if (tx.maxFeePerDataGas < dataGasPrice) {
                    throw new Error(`blob transaction maxFeePerDataGas ${tx.maxFeePerDataGas} < than block data gas price ${dataGasPrice} - ${this.errorStr()}`);
                }
            }
        }
    }
    ...
    validateUnclesHash() {
        ...
        const raw = rlp_1.RLP.encode((0, util_1.bufArrToArr)(uncles));
